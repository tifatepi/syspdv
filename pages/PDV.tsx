...
import { ShoppingCart, Search, Package, Plus, Minus, Trash2, X, CreditCard, Banknote, QrCode } from 'lucide-react';
...
const PDV: React.FC = () => {
  ...
  return (
    <div className="flex h-screen bg-[#f1f5f9] select-none">
      {/* Sidebar Categorias - Vertical para economizar altura */}
      ...
      {/* Direita: Carrinho e Checkout (Cupom Digital) */}
      <div className="w-[380px] bg-white border-l shadow-2xl flex flex-col no-print shrink-0">
        ...
        <div className="p-5 bg-slate-900 text-white rounded-t-[32px] shadow-[0_-10px_30px_rgba(0,0,0,0.1)]">
          ...
          <div className="flex gap-2">
             <button 
              onClick={() => { resetVenda(); playBeep('cancel'); }}
              className="flex-1 py-4 rounded-xl bg-slate-800 text-slate-400 hover:bg-red-500/20 hover:text-red-500 transition-all font-black text-[10px] uppercase tracking-widest"
            >
              LIMPAR (ESC)
            </button>
            <button 
              ...
              className="flex-[2] py-4 rounded-xl bg-blue-600 text-white shadow-lg shadow-blue-900/40 hover:bg-blue-500 transition-all font-black text-sm uppercase tracking-widest disabled:opacity-30 disabled:grayscale"
            >
              PAGAR (F12)
            </button>
          </div>
            ...
            <div className="flex-1 p-8">
              ...
              <div className="grid grid-cols-2 gap-3">
                {PAYMENT_METHODS.map(m => (
                  <button
                    key={m.id}
                    onClick={() => handleFinishSale(m.id)}
                    className={`flex items-center gap-3 p-5 rounded-2xl text-left border-2 border-slate-100 hover:border-blue-500 hover:bg-blue-50 transition-all group`}
                  >
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-white p-6 no-print overflow-y-auto">
          <div className="flex flex-col items-center gap-6 max-w-sm w-full text-center">
             ...
             <div className="bg-slate-50 p-6 rounded-2xl w-full font-mono text-left text-xs border border-dashed border-slate-300">
                ...
                <div className="border-t mt-2 pt-2 font-bold text-sm">
                   <div className="flex justify-between"><span>TOTAL</span> <span>R$ {lastSale.total.toFixed(2)}</span></div>
                </div>
                <p className="mt-4 text-[10px] opacity-60 text-center uppercase">{lastSale.timestamp}</p>
             </div>

             <div className="flex flex-col w-full gap-2">
                <button onClick={() => window.print()} className="w-full py-4 bg-slate-900 text-white rounded-xl font-black uppercase tracking-widest">Imprimir</button>
